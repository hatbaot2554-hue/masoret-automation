...
DASHBOARD_API = os.environ.get("DASHBOARD_API_URL", "https://masoret-dashboard.vercel.app/api")
...
def get_waitlist():
    res = requests.get(f"{DASHBOARD_API}/waitlist")
    if res.ok:
        return res.json()
    return []
...
def send_email(to_email, product_name, product_index):
    ...
    <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 32px; background: #F8F4EE;">
        ...
        <a href="{product_url}" style="display: inline-block; background: #C9A84C; color: #1A2332; padding: 14px 32px; text-decoration: none; font-weight: 700; font-size: 16px; border-radius: 4px; margin-top: 8px;">
          לרכישה ←
        </a>
      ...
      <div style="background: #1A2332; padding: 16px; text-align: center;">
        <p style="color: rgba(255,255,255,0.5); font-size: 12px; margin: 0;">
          © המרכז למסורת יהודית | 
          <a href="https://masoret-website.vercel.app" style="color: #C9A84C;">לאתר</a>
        </p>
      </div>
    </div>
    """
...
def main():
    ...
    for entry in waitlist:
        ...
        if in_stock is not False:
            print(f"שולח מייל ל-{email} על {product.get('name', '')}")
            success = send_email(email, product.get("name", ""), product_index)
            if success:
                notified.append(entry.get("id"))
                print(f"✅ נשלח בהצלחה")
            else:
    ...
    if notified:
        requests.post(
            f"{DASHBOARD_API}/waitlist/cleanup",
            json={"ids": notified}
        )
        print(f"נמחקו {len(notified)} רשומות מרשימת ההמתנה")
